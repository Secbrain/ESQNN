...
from torchquantum.plugins import tq2qiskit, tq2qiskit_parameterized, \
    tq2qiskit_measurement, append_fixed_gate
...
from torchquantum.plugins.qiskit_processor import EmptyPassManager, run_job_worker
from models.json_noise_model import IBMBackend_NoiseModel

IBMQ_NAMES = ['ibmq_lima', 'ibmq_belem', 'ibmq_manila', 'ibm_nairobi','ibm_oslo', 'ibmq_quito']

def qiskit_assemble_circs_new(encoders, fixed_layer, measurement):
    qc = QuantumCircuit(measurement.num_qubits, measurement.num_clbits)
    circs_all = []

    for encoder in encoders:
        circ = encoder + fixed_layer + measurement
        circs_all.append(circ)

    return circs_all

def op_history2qiskit_expand_params_new(n_wires, op_history, bsz):
    """ convert a tq op_history to a qiskit QuantumCircuit
        FIX: add some no parameters gates to the circuit
    Args:
    ...
    for i in range(bsz):
        ...
        for op in op_history:
            if op["params"] is not None:
                assert bsz == len(op["params"])
                append_fixed_gate(
                    circ, op["name"], op["params"][i], op["wires"], op["inverse"]
                )
            else:
                # add some no parameters gates to the circuit
                append_fixed_gate(
                    circ, op["name"], op["params"], op["wires"], op["inverse"]
                )
...
class QiskitNoiseProcessor(object):
    def __init__(self,
                 use_real_qc=False,
                 backend_name=None,
        ...
        self.noise_json_path = noise_json_path
        # self.noise_category = noise_category

        self.qiskit_init()

    def get_noise_model(self, name):
        print("processor.get_noise_model...")
        if name in IBMQ_NAMES:
            # backend = self.provider.get_backend(name)
            backend = IBMQ.get_provider().get_backend(name)
            # self.properties = backend.properties()
            if self.noise_json_path is not None:
                print("==== using IBMBackend_NoiseModel noise_json_data=====")
                # import pdb
                # pdb.set_trace()
                noise_model = IBMBackend_NoiseModel.from_backend_json(self.noise_json_path, backend)
                if self.noise_category == 'all':
                    print(f"Add noise: all, backned_json: {backend}")
                    self.noise_model = IBMBackend_NoiseModel.from_backend_json(self.noise_json_path, backend)

                elif self.noise_category == 'gate_readout':
                    print(f"Add noise: gate_readout, backned_json: {backend}")
                    self.noise_model = IBMBackend_NoiseModel.from_backend_json(self.noise_json_path, backend,
                                                thermal_relaxation=False)
                elif self.noise_category == 'gate_thermal':
                    print(f"Add noise: gate_thermal, backned_json: {backend}")
                    self.noise_model = IBMBackend_NoiseModel.from_backend_json(self.noise_json_path, backend, 
                                                readout_error=False)
                elif self.noise_category == 'gate_only':
                    print(f"Add noise: gate_only, backned_json: {backend}")
                    self.noise_model = IBMBackend_NoiseModel.from_backend_json(self.noise_json_path, backend, 
                            readout_error=False,   thermal_relaxation=False)
                elif self.noise_category == 'no_noise':
                    print(f"Don't Add noise, backned_json: {backend}")
                    self.noise_model = IBMBackend_NoiseModel.from_backend_json(self.noise_json_path, backend, 
                            gate_error=False, readout_error=False,   thermal_relaxation=False)
                else:
                    print(f"Noise Json model error")
                    print(f"noise_category: {self.noise_category}")
        else:
            noise_model = None
    ...
    def qiskit_init(self):
        ...
        if self.use_real_qc:
            ...
            self.coupling_map = self.get_coupling_map(self.backend_name)
            self.basis_gates = self.get_basis_gates(self.backend_name)
