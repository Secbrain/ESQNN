...
from models.circ_layers import Op1QAllLayer_R
...
class GeneralEncoder(tq.QuantumModule):
...
class PhaseEncoder(tq.QuantumModule):
    def __init__(self, arch):
        super().__init__()
        self.func = arch['func']
        self.enc_blocks = arch['enc_blocks']
    ...
    def forward(self, q_device: tq.QuantumDevice, x):
        self.q_device = q_device
        self.q_device.reset_states(x.shape[0])
        for k in range(self.q_device.n_wires * 4 * self.enc_blocks):
            self.func(self.q_device, wires=k%4, params=x[:, k],
                      static=self.static_mode, parent_graph=self.graph)


class MultiPhaseEncoder(tq.QuantumModule):
    def __init__(self, arch):
        super().__init__()
        self.funcs = arch['funcs'] if isinstance(arch['funcs'], Iterable) else [arch['funcs']]
        if 'wires' in arch.keys():
            self.wires = arch['wires']
    ...
    def forward(self, q_device: tq.QuantumDevice, x):
        ...
        self.q_device = q_device
        self.q_device.reset_states(x.shape[0])
        # x = normal_data(x, min_bound=1e-8, max_bound=np.pi)
...
class AmplitudeEncoder(Encoder, metaclass=ABCMeta):
    # AmplitudeEncoder
    def __init__(self, arch):
        super().__init__()
    ...
    def forward(self, q_device: tq.QuantumDevice, x):
        ...
        # normalize the input
        x = x / (torch.sqrt((x.abs()**2).sum(dim=-1))+1e-18).unsqueeze(-1)
...
class StateEncoder_New(tq.QuantumModule):
    def __init__(self, arch):
        super().__init__()
    
    def forward(self, q_device: tq.QuantumDevice, x):
        self.q_device = q_device
        """ pre process data : 1) >=1.2: 1 2) <1.2:0
            transforms.Normalize((0.1307,), (0.3081,))] 
        """
        # 对于一些最大值不及1.2的值进行变化
        has_gt_1_2 = torch.any(x>=1.2, dim=1)
        max_value, max_loc = torch.max(x, dim=1)
        x[~has_gt_1_2, max_loc[~has_gt_1_2]] = 1.2
        # 重新赋值，基态编码
        x[x<1.2] = 0 # 注意顺序
        x[x>=1.2] = 1
        # normalize
        x = x / (torch.sqrt((x.abs()**2).sum(dim=-1))+1e-18).unsqueeze(-1)
        state = x.view([x.shape[0]] + [2] * self.q_device.n_wires)
        self.q_device.states = state.type(C_DTYPE)

class StateEncoder(tq.QuantumModule):
    def __init__(self, arch):
        super().__init__()
...
class IQPEncoder(tq.QuantumModule):
    """  
        gate composition: H RZ RZZ(CNOT, RZ, CNOT)
        default Rzz wires_list: [[0,1], [1, 2], [2, 3]...[wire_num-2, wire_num-1]]
    """
    def __init__(self, arch):
        super().__init__()
        self.n_wires = arch['n_wires']
        self.enc_blocks = arch['enc_blocks']
        self.wires_list = [[0,1], [1, 2], [2, 3]] if 'wires_list' not in arch.keys() else arch['wires_list']
        self.iqp_h_circ = tq.QuantumModuleList()
        self.iqp_r_circ = tq.QuantumModuleList()
        self.iqp_circ_rzz = tq.QuantumModuleList()
        for i in range(arch['enc_blocks']*4):
            self.iqp_h_circ.append(tq.Op1QAllLayer(op=tq.H, \
                        n_wires=self.n_wires, has_params=False, trainable=False))
            
            for k in range(self.n_wires):
                if arch['rotation_gate']=='rz':
                    self.iqp_r_circ.append(tq.RZ())
                elif arch['rotation_gate']=='rx':
                    self.iqp_r_circ.append(tq.RX())
                elif arch['rotation_gate']=='ry':
                    self.iqp_r_circ.append(tq.RY())
            # not circular rzz
            for k in range(self.n_wires-1):
                if arch['rotation_gate']=='rz':
                    self.iqp_circ_rzz.append(tq.RZZ())
                elif arch['rotation_gate']=='rx':
                    self.iqp_circ_rzz.append(tq.RXX())
                elif arch['rotation_gate']=='ry':
                    self.iqp_circ_rzz.append(tq.RYY())
    
    @tq.static_support
    def forward(self, q_device: tq.QuantumDevice, x):
        self.q_device = q_device
        self.q_device.reset_states(x.shape[0])
        bsz = x.shape[0] # bsz, enc_blocks * 4
        # x = normal_data(x) # 将值规范到0 ~ 360，便于旋转角度的设置

        for k in range(self.enc_blocks * 4):
            self.iqp_h_circ[k](self.q_device)

            for i in range(self.n_wires):
                self.iqp_r_circ[i+4*k](self.q_device, wires=i, params=x[:, k*4+i].reshape(-1, 1))

            # calculate rzz_params
            for i in range(self.n_wires-1):
                self.iqp_circ_rzz[i+(self.n_wires-1)*k](self.q_device,\
                                    wires=self.wires_list[i], \
                                    params=x[:, i+k*4]*x[:, k*4+i+1])
...
encoder_circ_dict = {
    ...
    'state_new': StateEncoder_New, 
    'state': StateEncoder,
    'phase': PhaseEncoder, 
    'general': GeneralEncoder, 
    'multiphase': MultiPhaseEncoder, 
    'multiphase_z': MultiPhaseEncoder, 
    'multiphase_x': MultiPhaseEncoder, 
    'IQP': IQPEncoder,  
    'IQP_X': IQPEncoder, 
    'IQP_Y': IQPEncoder,
}
